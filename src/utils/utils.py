...
def med_reshape(image, new_shape):
    ...
    # If the new shape is larger in every dimension
    if original_shape <= new_shape:
        reshaped_image = np.zeros(new_shape, dtype=image.dtype)
        reshaped_image[:original_shape[0], :original_shape[1], :original_shape[2]] = image
    # If the new shape is smaller in any dimension
    else:
        skip_factors = [int(original_dim/new_dim) for original_dim, new_dim in zip(original_shape, new_shape)]
        reshaped_image = image[::skip_factors[0], ::skip_factors[1], ::skip_factors[2]]

    if reshaped_image.shape[1:3] != (64, 64):
        print("Shape not 64*64: ", reshaped_image.shape)
