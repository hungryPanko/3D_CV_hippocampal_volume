...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        self.model_path = ""
        #self.model_path = "/Users/hannalee/Documents/Udacity/AI_healcare/3D_FinalProj/section2/result/2023-08-23_1957_Basic_unet"
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            data = batch["image"].squeeze(2).float().to(self.device)
            target = batch["seg"].squeeze(2).to(self.device)
            #print(data.shape, target.shape ) #torch.Size([8, 1, 64, 64]) torch.Size([8, 1, 64, 64])
            ...
            # ANSWER:
            # The dimensions of the 'prediction' variable represent:
            # 1. BATCH_SIZE: Number of samples in the batch
            # 2. Number of output channels (e.g., classes for segmentation tasks)
            # 3. PATCH_SIZE: Height of the image patch
            # 4. PATCH_SIZE: Width of the image patch
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_loader):
                ...
                # <YOUR CODE HERE>
                # Extracting image and segmentation slices from the batch.
                # data = batch["image"]
                ...
                # We get the softmax'd version of the prediction to output a probability map.
                prediction_softmax = F.softmax(prediction, dim=1)

                # Calculating the loss using predictions and true segmentations
                loss = self.loss_function(prediction, target[:, 0, :, :])
    ...
    def load_model_parameters(self, path=''):
        ...
        """
        if not path:
            self.model_path = os.path.join(self.out_dir, "model.pth")
        else:
        ...
        if os.path.exists(self.model_path):
            ...
            raise Exception(f"Could not find path {self.model_path}")
    ...
    def run_test(self):
        ...
        inference_agent = UNetInferenceAgent(parameter_file_path=self.model_path, model=self.model, device=self.device)
    ...
    def run(self):
        ...
        # If a saved model exists, load it and skip training. Else, proceed with training.
