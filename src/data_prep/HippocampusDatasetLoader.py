...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        if image.shape[1:3] != (64, 64):
            print("Shape not 64*64: ", image.shape)
            print("image: ",f)


        # TASK: Why do we need to cast label to int?
        # ANSWER: Casting labels to integers is important because labels represent distinct classes. 
        # Using floating-point numbers for class labels could introduce inaccuracies
        # due to the way floating-point arithmetic works. 
        # Also, most neural network frameworks and functions for calculating metrics 
        # or loss expect class labels to be integers.
