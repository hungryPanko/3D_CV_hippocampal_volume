...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Get the 3D volume id and the slice id from the tuple
        vol_id, slice_id = slc
        
        # Extract 2D slices for both image and segmentation
        image_slice = self.data[vol_id]["image"][slice_id]
        seg_slice = self.data[vol_id]["seg"][slice_id]

        # Add an additional dimension and convert them to Torch tensors
        sample["image"] = torch.from_numpy(image_slice[None, :])
        sample["seg"] = torch.from_numpy(seg_slice[None, :])
